...
class EntityCache {
    ...
    CacheImpl getCacheOneBf() { return efi.ecfi.getCacheFacade().getCacheImpl("entity.${efi.tenantId}.one_bf") }
    CacheImpl getCacheList(String entityName) { return efi.ecfi.getCacheFacade().getCacheImpl("entity.${efi.tenantId}.list.${entityName}") }
    CacheImpl getCacheListRa(String entityName) { return efi.ecfi.getCacheFacade().getCacheImpl("entity.${efi.tenantId}.list_ra.${entityName}") }
    ...
    void clearCacheForValue(EntityValueBase evb, boolean isCreate) {
        try {
            ...
            // clear one cache
            if (efi.ecfi.getCacheFacade().cacheExists("entity.${efi.tenantId}.one.${entityName}")) {
    ...
    void registerCacheListRa(String entityName, EntityCondition ec, EntityListImpl eli) {
        ...
        if (ed.isViewEntity()) {
            ...
            for (Node memberEntityNode in ed.getEntityNode()."member-entity") {
                ...
                Cache listViewRaCache = getCacheListRa((String) memberEntityNode.'@entity-name')
